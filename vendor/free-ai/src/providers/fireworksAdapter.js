export class FireworksAdapter {
  ...
  async call(modelId, compiledPrompt, ctx, opts = { timeout: 15000 }) {
    ...
    try {
      ...
      return { ok: true, text, model_id: modelId, provider_id: 'fireworks', http_status: status, finish_reason: j?.choices?.[0]?.finish_reason || null, usage: j?.usage || null, raw_error: null, latency_ms: 0 };
    } catch (e) {
