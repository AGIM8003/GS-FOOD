export class HuggingFaceAdapter {
  ...
  async call(modelId, compiledPrompt, ctx, opts = { timeout: 15000 }) {
    ...
    const body = { model: modelId, messages: [{ role: 'user', content: compiledPrompt }], stream: false };
    ...
    try {
      ...
      return { ok: true, text, model_id: modelId, provider_id: 'huggingface', http_status: status, finish_reason: j?.choices?.[0]?.finish_reason || null, usage: j?.usage || null, raw_error: null, latency_ms: 0 };
    } catch (e) {
